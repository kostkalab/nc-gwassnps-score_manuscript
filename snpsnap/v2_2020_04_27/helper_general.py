...
class Outputs:
    ...
    def add_output(self, output, output_path, mkdir=False, add_root=False, custom_root=None):
        ...
        if add_root and not custom_root:
            ...
            raise ValueError('two root paths were provided when creating output files/dirs.')
        else:
...
def safe_mkdir(path):
    ...
    if not os.path.isdir(path):
        ...
        print("Note: {} already exists. output will overwrite".format(path))
...
def report_iferr(plinkcmd, decode_stdout, err_bool, logger=None, type="Error"):
    ...
    if np.any(err_bool):
        ...
        if logger:
            logger.debug(f"{type} while running plink cmd:\n  {plinkcmd}\n  {decode_stdout[np.where(err_bool)[0][0]]}\n")
...
def warning_check_plink_run(plink_stdout, plink_stderr, plinkcmd, logger=None):
    ...
    # report
    report_iferr(plinkcmd, warnings_stdout, warnings_bool, logger=logger, type="Warning")
    report_iferr(plinkcmd, warnings_stderr, warnings_bool_stderr, logger=logger, type="Warning")
