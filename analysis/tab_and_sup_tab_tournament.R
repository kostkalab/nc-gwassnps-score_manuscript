...
p_value = read_csv("./sup_data/sup_data_pairwise-org-aggregated.csv.gz")
p_value_by_disease = read_csv("./sup_data/sup_data_pairwise-org-individual.csv.gz")
...
p_value$higher_median = factor(p_value$higher_median, levels = c("CADD", "eigen", "GWAVA", "GenoCanyon", "LINSIGHT"))
p_value$lower_median = factor(p_value$lower_median, levels = c("CADD", "eigen", "GWAVA", "GenoCanyon", "LINSIGHT"))
...
p_value_by_disease$higher_median = factor(p_value_by_disease$higher_median, levels = c("CADD", "eigen", "GWAVA", "GenoCanyon", "LINSIGHT"))
p_value_by_disease$lower_median = factor(p_value_by_disease$lower_median, levels = c("CADD", "eigen", "GWAVA", "GenoCanyon", "LINSIGHT"))
...
format_df = function(df_pr){
  ...
  df_pr[,5] = apply(df_pr, 1, function(x) 4-x[1]-x[2])
  df_pr[,6] = apply(df_pr, 1, function(x) 444-x[3]-x[4])
...
write_csv(df_pr,"./sup_data/tab_tournament.csv")
write_csv(df_roc, "./sup_data/sup_tab_tournament.csv")
